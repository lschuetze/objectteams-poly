...
//{ObjectTeams: variant for guaranteed pure java:
	public static IStatus validateIdentifier(String id, String sourceLevel, String complianceLevel, boolean pureJava) {
		synchronized (SCANNER) {
			boolean old = SCANNER.parsePureJavaOnly;
			try {
				SCANNER.parsePureJavaOnly = pureJava;
...
//{ObjectTeams: package names may contain OT keywords:
/* orig:
  ...
  :giro */
			...
			synchronized (SCANNER) {
				...
				try {
					SCANNER.parsePureJavaOnly = true;
					scannedID = scannedIdentifier(typeName, sourceLevel, complianceLevel);
				} finally {
					SCANNER.parsePureJavaOnly = old;
				}
			}
