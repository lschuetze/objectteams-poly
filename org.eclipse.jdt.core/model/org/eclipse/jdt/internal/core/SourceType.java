		...
		case OTJavaElement.OTEM_METHODMAPPING:
			...
			boolean isSetter = false;
			{
				...
				switch(bindingKind.charAt(0)) {
					...
					case 'r': kind = IOTJavaElement.CALLIN_MAPPING; callinKind = ICallinMapping.KIND_REPLACE; break;
					case 'O': isOverride = true; //$FALL-THROUGH$
					case 'o': kind = IOTJavaElement.CALLOUT_MAPPING; break;
					case 'G': isOverride = true; //$FALL-THROUGH$
					case 'g': kind = IOTJavaElement.CALLOUT_TO_FIELD_MAPPING; break;
					case 'S': isOverride = true; //$FALL-THROUGH$
					case 's': kind = IOTJavaElement.CALLOUT_TO_FIELD_MAPPING; isSetter = true; break;
				}
			...
			if (kind == IOTJavaElement.CALLIN_MAPPING) {
				bindingName = memento.nextToken();
				if (bindingName.charAt(0) == '<') {
					StringBuffer buf = new StringBuffer();
					buf.append(bindingName);
					String next = memento.nextToken();
					while (next.charAt(0) != OTJavaElement.OTEM_METHODMAPPING) {
						buf.append(next);
			...
			if (kind != IOTJavaElement.CALLOUT_TO_FIELD_MAPPING) {
				String tok = memento.nextToken();
				while (tok.charAt(0) != OTJavaElement.OTEM_METHODMAPPING) {
					baseMethods.add(MethodMapping.createMethodData(memento, tok));
			...
			case IOTJavaElement.CALLOUT_TO_FIELD_MAPPING:
				...
				return new CalloutToFieldMapping(0, 0, 0, 0, this, corrJavaMethod, roleMethod, baseField, hasSign, isOverride, 0/*declaredModifiers*/, false/*addAsChild*/);
			default:
