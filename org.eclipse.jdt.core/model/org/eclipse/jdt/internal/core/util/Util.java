 ...
 * $Id: Util.java 23404 2010-02-03 14:10:22Z stephan $
 *
 * Contributors:
 ...
 * <h4>OTDT changes:</h4>
 * <dl>
 * <dt>What:<dd> don't return null when findFirstClassFile looks at a .svn folder before real class folders
 *
 * <dt>What:<dd> respect value dependent types.
 * <dt>How:<dd>  don't print &lt;&gt; if <em>only</em> a value parameter is given.
 *
 * <dt>What:<dd> respect lifting type and type anchor ({@link #getFullyQualifiedName}).
 * </dl>
 * <hr>
...
//{ObjectTeams: (we saw tests failing due to the existence of .svn directories)
/* orig:
					return findFirstClassFile((IFolder)member);
  :giro */
					IFile cf = findFirstClassFile((IFolder)member);
					if (cf != null)
						return cf;
// SH}
...
//{ObjectTeams: if type had only value parameters don't print empty <>:
...
// SH}
			...
			case ASTNode.WILDCARD_TYPE:
				...
				break;
//{ObjectTeams: new kinds of types:
			case ASTNode.LIFTING_TYPE:
				getFullyQualifiedName(((LiftingType)type).getBaseType(), buffer);
				break;
			case ASTNode.TYPE_ANCHOR:
				buffer.append('@');
				buffer.append(((TypeAnchor)type).getPath().getFullyQualifiedName());
				break;
// SH}
		}
	}
...
//{ObjectTeams: do not read the file of an enclosing team, if we actually look for a role file:
/* orig:
			if (declaringTypeBinding == null) {
  :giro */
			if (   declaringTypeBinding == null
				|| !CharOperation.equals(((ReferenceBinding)declaringTypeBinding).getFileName(), fileName))
			{
// SH}
				// top level type
				if (org.eclipse.jdt.internal.compiler.util.Util.isClassFileName(fileName)) {
...
//{ObjectTeams: force parser to parsePureJavaOnly:
/* orig:
		return JavaConventions.validateIdentifier(folderName, sourceLevel, complianceLevel).getSeverity() != IStatus.ERROR;
  :giro */
		return JavaConventions.validateIdentifier(folderName, sourceLevel, complianceLevel, true/*pureJava*/).getSeverity() != IStatus.ERROR;
// SH}
	}
