...
//{ObjectTeams: new node types
    ...
    public boolean visit(CallinMappingDeclaration node) {
		...
		if (name != null && found(node, name) && this.resolveBinding)
			this.foundBinding = node.resolveBinding();
		else if (found(node, node.getRoleMappingElement()) && this.resolveBinding)
			this.foundBinding = node.resolveBinding();
		return true;
    }
    @Override
    public boolean visit(CalloutMappingDeclaration node) {
		if (found(node, node.getRoleMappingElement()) && this.resolveBinding)
			this.foundBinding = node.resolveBinding();
		return true;
    }
// SH}
