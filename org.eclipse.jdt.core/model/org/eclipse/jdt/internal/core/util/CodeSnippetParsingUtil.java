...
/**
 * OTDT changes:
 * new methods parseParameterMapping.
 *
...
//{ObjectTeams: a separate method parseParameterMapping, because of separate parser.parseParameterMapping method
	public Expression parseParameterMapping(char[] source, Map settings, boolean recordParsingInformation) {
		return parseParameterMapping(source, 0, source.length, settings, recordParsingInformation);
	}

	public Expression parseParameterMapping(char[] source, int offset, int length, Map settings, boolean recordParsingInformation) {
		...
		CompilationResult compilationResult = new CompilationResult(sourceUnit, 0, 0, compilerOptions.maxProblemsPerUnit);
		CompilationUnitDeclaration unit = new CompilationUnitDeclaration(problemReporter, compilationResult, source.length);
		Expression result = parser.parseParameterMapping(source, offset, length, unit);
