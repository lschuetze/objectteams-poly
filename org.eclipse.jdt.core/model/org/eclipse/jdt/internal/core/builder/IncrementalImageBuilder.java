 ...
 * $Id: IncrementalImageBuilder.java 23401 2010-02-02 23:56:05Z stephan $
 *
 * Contributors:
 ...
 * <dt>How:<dd> strip overlap between package path and type name (the team package)
 *
 * <dt>What:<dd> provide hook  {@link #shouldPreserveBinary} and API {@link #scheduleForRemoval}
 *               to the compiler.adaptor.
 * </dl>
 * <hr>
	...
	next : for (int i = 0, l = previousTypeNames.length; i < l; i++) {
		...
		for (int j = 0, m = definedTypeNames.size(); j < m; j++)
			if (CharOperation.equals(previous, (char[]) definedTypeNames.get(j)))
				continue next;
		SourceFile sourceFile = (SourceFile) result.getCompilationUnit(); // go directly back to the sourceFile
		if (packagePath == null) {
...
//{ObjectTeams: inner classes whose binaries have successfully been reused are not to be deleted!
		if (shouldPreserveBinary(result, sourceFile.sourceLocation.sourceFolder.getFullPath(), packagePath, previous))
			continue next;
// SH}
...
//{ObjectTeams: Re-used binary roles should not be deleted.
//              Actually more conditions come into play, which will be hooked
//              into this join point by the BuildManager team.
public boolean shouldPreserveBinary(
		CompilationResult cResult, IPath sourceFolder, IPath packagePath, char[] binaryTypeName)
{
	return cResult.hasBinaryMember(binaryTypeName);
}
/** entry for the compiler.adaptor to tell us about class files that need to be removed: */
public void scheduleForRemoval(SourceFile sourceFile, String typePath) {
	// copied mostly from finishedWith:
	if (this.secondaryTypesToRemove == null)
		this.secondaryTypesToRemove = new SimpleLookupTable();
	ArrayList<IPath> types = (ArrayList) this.secondaryTypesToRemove.get(sourceFile.sourceLocation.binaryFolder);
	if (types == null)
		types = new ArrayList<IPath>(1);
	types.add(new Path(typePath));
	if (typePath.contains(IOTConstants.OT_DELIM))
		types.add(new Path(typePath.replace(IOTConstants.OT_DELIM, ""))); // ifc-part, too. //$NON-NLS-1$
		// FIXME(SH): take care to remove __OT__ only for the innermost type name.
	this.secondaryTypesToRemove.put(sourceFile.sourceLocation.binaryFolder, types);
}
// SH}
...
//{ObjectTeams: check if package name of a role file is duplicated:
		return;
	}
	if (typePath.segmentCount() < 2)
		return;
	String typeName= typePath.lastSegment();
	typePath= typePath.removeLastSegments(1);
	String lastPackage= typePath.lastSegment();
	typePath= typePath.removeLastSegments(1);
	if (typeName.startsWith(lastPackage+'$')) {
		typePath= typePath.append(typeName);
		// try again
		removeClassFile(typePath, outputFolder);
// SH}
	}
}
