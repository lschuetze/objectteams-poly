 ...
 * $Id: AbstractImageBuilder.java 22741 2009-10-13 22:23:05Z stephan $
 *
 * Contributors:
...
//{ObjectTeams: ROFI role files are not treated as nested here:
			if (result.roleFileDepth > 0) isNestedType = false;
// SH}
...
//{ObjectTeams:
protected SourceFile findSourceFile(IPath path) {
	Object[] files = this.workQueue.needsCompileList.values;

	for(int i=0; i< files.length; i++) {
		if (files[i] == null) continue;
		SourceFile file = (SourceFile) files[i];
		if (pathsMatch(file.getPath(), path))
			return file;
	}
	files = this.workQueue.compiledList.values;
...
private boolean pathsMatch(IPath relative, IPath absolute) {
	String[] relSegments = relative.segments();
	int offset = absolute.segmentCount()-relSegments.length;
	if (offset < 0)
		return false;
	for (int i = 0; i < relSegments.length; i++) {
		if (!relSegments[i].equals(absolute.segment(i+offset)))
			return false;
	}
	return true;
}
...
//{ObjectTeams: store path if needed:
	classFile.maybeSetClassFilePath(file.getLocation().toOSString());
// SH}
