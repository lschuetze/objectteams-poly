...
public SourceType[] getMemberTypeHandles() {
	...
	for (int i = 0; i < length; i++) {
		IJavaElement child = this.children[i];
//{ObjectTeams: need to normalize because OTType is not SourceType!
		if (child instanceof OTType)
			child = ((OTType)child).getCorrespondingJavaElement();
// SH}
...
public boolean isValueParameterAt(int i) {
	if (i < this.typeParameters.length)
		return ((TypeParameter)this.typeParameters[i]).isValueParameter;
	return false;
}
// SH}
/**
