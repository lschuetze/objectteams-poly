...
//{ObjectTeams: filter recording (cf. CompilationUnitProblemFinder):
			if (context instanceof AbstractMethodDeclaration) {
				...
				if (method.isGenerated) {
					// might also compare the CUD to the requester's.parser's.CUD?
...
//{ObjectTeams: references in @role tags:
		TypeReference[] roleReferences = this.javadoc.roleReferences;
		if (roleReferences != null) {
			for (int i = 0, max=roleReferences.length; i < max; i++) {
				TypeReference typeRef = roleReferences[i];
				if (typeRef instanceof JavadocSingleTypeReference) {
					JavadocSingleTypeReference singleRef = (JavadocSingleTypeReference) typeRef;
					this.requestor.acceptTypeReference(singleRef.token, singleRef.sourceStart);
				} // qualified type reference not supported here
			}
		}
// SH}
...
//OT/J:		specific flag AccBase:
	pushOnAstStack(impt = newImportReference(tokens, positions, false, ExtraCompilerModifiers.AccBase));
...
protected void consumeClassHeaderPlayedBy() {
	...
	this.requestor.acceptBaseReference(
            baseclassRef.getTypeName(),
            baseNameStart,
            baseNameEnd);
}
//haebor}
