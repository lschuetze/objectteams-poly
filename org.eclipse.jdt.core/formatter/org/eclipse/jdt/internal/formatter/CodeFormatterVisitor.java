 ...
 * $Id: CodeFormatterVisitor.java 23404 2010-02-03 14:10:22Z stephan $
 *
 * Contributors:
 ...
 * OTDT changes:
 * Added
 * + formatting of method mappings
 * + visit methods for all OT elements
 * 
 * TODO(jsv) add OT elemets to the code style preferences page and change the corresponding formatter methods
 *   
...
// support formatting non-OT/J code? 		
		...
		if (CompilerOptions.DISABLED.equals(otjOption))
			this.preferences.scopedKeywords= false;
...
//{ObjectTeams: don't format generated methods!
		if (methodDeclaration.isGenerated)
			return;
// SH}
...
//{ObjectTeams: - format Method for AbstractMethodMappingDeclarations
//	            - copy of format method for AbstractMethodDeclaration
// TODO(jsv) use OT code style
	private void format(
...
//{ObjectTeams: another modifier:
		if (importRef.isBase()) {
			if (this.preferences.scopedKeywords || this.preferences.isPureJava) // base has different token type depending on scanner mode:
				this.scribe.printNextToken(TerminalTokens.TokenNameIdentifier); // not in OT/J mode before start of "team class"
			else
				this.scribe.printNextToken(TerminalTokens.TokenNamebase); // unconditionally in this mode
			this.scribe.space();
		}
// SH}
...
//{ObjectTeams: handle OT/J options:
...
//{ObjectTeams: extracted to handle options scopedKeywords/pureJava:		
	private void configureKeywordMode(final char[] compilationUnitSource) {
		// The additional check:		
		if (this.preferences.scopedKeywords) {
			this.localScanner.parseOTJonly = false;
			this.scribe.scanner.parseOTJonly = false;
		}
		if (this.preferences.isPureJava) {
			this.localScanner.parsePureJavaOnly = true;
			this.scribe.scanner.parsePureJavaOnly = true;
		}
	}
//SH}
...
//{ObjectTeams: format playedBy token and the name of the bound base class		
		/*
		 * Role
		 */
		if (typeDeclaration.baseclass != null)
		{
			this.scribe.printNextToken(TerminalTokens.TokenNameplayedBy,true);
			this.scribe.space();
			typeDeclaration.baseclass.traverse(this, typeDeclaration.scope);
		}
		/* team or role: predicate */
		if (typeDeclaration.predicate != null) 
			formatGuardPredicate(typeDeclaration.predicate);
//jsv+SH}	
...
//{ObjectTeams:  AbstractMethodMappingDeclaration is also a BodyDeclaration and has to be formatted					
					//TODO(jsv) add own Type e.g. CALLOUT / CALLIN or METHODMAPPING
					} else if (member instanceof AbstractMethodMappingDeclaration) {
						isChunkStart = memberAlignment.checkChunkStart(METHOD, i, this.scribe.scanner.currentPosition);
						format((AbstractMethodMappingDeclaration) member, null, isChunkStart, i == 0);
//jsv}				
					} else {
...
//{ObjectTeams: visit and format methods for OT Elements	
	public boolean visit(CallinMappingDeclaration callinMappingDeclaration, ClassScope scope)
	{
		if (callinMappingDeclaration.ignoreFurtherInvestigation) {
			this.scribe.printComment();
			this.scribe.scanner.resetTo(callinMappingDeclaration.sourceEnd + 1, this.scribe.scannerEndPosition);
			this.scribe.printComment(CodeFormatter.K_UNKNOWN, Scribe.BASIC_TRAILING_COMMENT);
			return false;
		}
        ...
        {
        	roleMethodSpec.traverse(this,callinMappingDeclaration.scope);
        }

        boolean insertLineBreaks = false;
        if (this.scribe.line > startLine) {
        	this.scribe.printNewLine();
        	insertLineBreaks = true;
        }

		this.scribe.printNextToken(TerminalTokens.TokenNameBINDIN,true);
		this.scribe.printNextToken(TerminalTokens.TokenNameIdentifier, true);
		if (insertLineBreaks)
			this.scribe.printNewLine();
		else
        ...
        if (baseMethodSpecs != null)
        {
        	//baseMethodSpec.traverse(this,callinMappingDeclaration.scope);
        	formatMethodSpecs(
        ...
        if (callinMappingDeclaration.predicate != null)
        	formatGuardPredicate(callinMappingDeclaration.predicate);
		
        // format ParameterMappings
        if (callinMappingDeclaration.mappings != null &&
        	callinMappingDeclaration.mappings.length>0 )
        {
        	formatParameterMappings(callinMappingDeclaration, scope);
        }
	...
	{
		if (calloutMappingDeclaration.ignoreFurtherInvestigation) {
			this.scribe.printComment();
			this.scribe.scanner.resetTo(calloutMappingDeclaration.sourceEnd + 1, this.scribe.scannerEndPosition);
			this.scribe.printComment(CodeFormatter.K_UNKNOWN, Scribe.BASIC_TRAILING_COMMENT);
			return false;
		}
        ...
        if (roleMethodSpec != null)
        {
        	roleMethodSpec.traverse(this,calloutMappingDeclaration.scope);
        }
		...
		if (calloutMappingDeclaration.isCalloutOverride())
		{
			this.scribe.printNextToken(TerminalTokens.TokenNameCALLOUT_OVERRIDE,true);
		}
		else
		{
			this.scribe.printNextToken(TerminalTokens.TokenNameBINDOUT,true);
		}
        ...
        {
        	baseMethodSpec.traverse(this,calloutMappingDeclaration.scope);
        }
		
        // format ParameterMappings
        if (calloutMappingDeclaration.mappings != null &&
        	calloutMappingDeclaration.mappings.length>0 )
        {
        	formatParameterMappings(calloutMappingDeclaration, scope);
        }
	...
	public boolean visit(MethodSpec methodSpec, BlockScope scope)
	{
		if (methodSpec instanceof FieldAccessSpec)
		{
			...
			FieldAccessSpec fieldAccessSpec = (FieldAccessSpec)methodSpec;
			// Note(SH): printNextToken starts with scribe.printComment()
			// whereby we loose the calloutSeen flag, thus get/set are read as identifiers:
			this.scribe.printNextToken(TerminalTokens.TokenNameIdentifier, true);
			this.scribe.space();
			
			if (fieldAccessSpec.hasSignature)
			{
				// print field type, if signature exists
				final TypeReference returnType = fieldAccessSpec.returnType;
				if (returnType != null) {
		...
		{
			// print return type, if signature exists
			final TypeReference returnType = methodSpec.returnType;
			if (returnType != null) {
				returnType.traverse(this, scope);
			}
		}
	...
	{
		if (mapping.direction == TerminalTokens.TokenNameBINDIN)
		{
			this.scribe.printNextToken(TerminalTokens.TokenNameIdentifier);
			this.scribe.printNextToken(TerminalTokens.TokenNameBINDIN, true);
			this.scribe.space();
			try { // support 'base' as identifier in base part
				this.scribe.scanner.forceBaseIsIdentifier();
				mapping.expression.traverse(this,scope);
			} finally {
				this.scribe.scanner.restoreBaseKeyword();
			}
		}
		if (mapping.direction == TerminalTokens.TokenNameBINDOUT)
		{
			 mapping.expression.traverse(this,scope);
			 this.scribe.printNextToken(TerminalTokens.TokenNameBINDOUT, true);
			 this.scribe.space();
	...
	public boolean visit(BaseCallMessageSend messageSend, BlockScope scope) 
	{
		...
		messageSend.getMessageSend().traverse(this,scope);
		return false;
	}
	
	public boolean visit(LiftingTypeReference liftingTypeReference, BlockScope scope) {
		liftingTypeReference.baseReference.traverse(this,scope);
		this.scribe.printNextToken(TerminalTokens.TokenNameas,true);
		this.scribe.space();
		liftingTypeReference.roleReference.traverse(this,scope);
		return false;
	}
	
	//TODO(jsv) do we really need the visit method for lifting type with class scope? 
	...
	public boolean visit(WithinStatement withinStatement, BlockScope scope) {
		this.scribe.printNextToken(TerminalTokens.TokenNamewithin);
		final int line = this.scribe.line;
		this.scribe.printNextToken(TerminalTokens.TokenNameLPAREN, this.preferences.insert_space_before_opening_paren_in_while); // TODO(SH): OT-option

		if (this.preferences.insert_space_after_opening_paren_in_while) {
			this.scribe.space();
		}
		withinStatement.getTeamExpression().traverse(this, scope);

		this.scribe.printNextToken(TerminalTokens.TokenNameRPAREN, this.preferences.insert_space_before_closing_paren_in_while); // TODO(SH): OT-option

        formatLeftCurlyBrace(line, this.preferences.brace_position_for_block);
    	Statement action = withinStatement.getAction();
		if (action != null) {
			if (action instanceof Block) {
                formatLeftCurlyBrace(line, this.preferences.brace_position_for_block);
				action.traverse(this, scope);
	...
	// TODO(jsv) use OT Code Style
	private void formatMethodSpecArguments(
		...
		final Argument[] arguments = methodSpec.arguments;
		if (arguments != null) {
			int argumentLength = arguments.length;
			Alignment argumentsAlignment = this.scribe.createAlignment(
					Alignment.METHOD_ARGUMENTS,
			...
			do {
				try {
					...
					for (int i = 0; i < argumentLength; i++) {
						...
						//TODO(jsv) MethodSpec does not have an own scope
						arguments[i].traverse(this, scope);
					}
					ok = true;
				} catch (AlignmentException e) {
	...
	//TODO (jsv) use OT format style for base methodSpecs
	private void formatMethodSpecs(
			CallinMappingDeclaration callinMappingDecl, 
			boolean spaceBetweenEmptyParameters,
			boolean spaceBeforeFirstParameter, 
			boolean spaceBeforeComma, 
			boolean spaceAfterComma,
		...
		final MethodSpec[] methodSpecs = callinMappingDecl.getBaseMethodSpecs();
		...
		do {
			try {
				...
				for (int i = 0; i < methodSpecLength; i++) {
					if (i > 0) {
						this.scribe.printNextToken(TerminalTokens.TokenNameCOMMA, spaceBeforeComma);
						this.scribe.printComment(CodeFormatter.K_UNKNOWN, Scribe.BASIC_TRAILING_COMMENT);
					}
					...
					methodSpecs[i].traverse(this, callinMappingDecl.scope);
				}
				ok = true;
			} catch (AlignmentException e) {
				this.scribe.redoAlignment(e);
	...
	// TODO(jsv) use OT style instead of method style
	private void formatParameterMappings(AbstractMethodMappingDeclaration abstractMethodMappingDeclaration,ClassScope scope)
	{
		this.scribe.printNextToken(TerminalTokens.TokenNamewith, true);
		final int line = this.scribe.line;
		...
		final ParameterMapping[] mappings = abstractMethodMappingDeclaration.mappings;
		if (mappings != null) {
			this.scribe.printNewLine();
			if (this.preferences.indent_statements_compare_to_body) {
				this.scribe.indent();
			...
			{
				final ParameterMapping mapping = mappings[idx];
				if (idx > 0) {
	...
	private void formatGuardPredicate(GuardPredicateDeclaration predicate) {
		// TODO(SH): replace hard code style with configurable option:
		this.scribe.printNewLine();
		this.scribe.indent();
		try {
			if (predicate.isBasePredicate) {
				this.scribe.printNextToken(TerminalTokens.TokenNamebase, true);
				this.scribe.space();
				this.scribe.scanner.forceBaseIsIdentifier();
			}
			this.scribe.printNextToken(TerminalTokens.TokenNamewhen, true);
			this.scribe.space();
			this.scribe.printNextToken(TerminalTokens.TokenNameLPAREN);
			predicate.expression().traverse(this, predicate.scope);
