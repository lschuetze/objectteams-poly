...
//{ObjectTeams: declaration needed for several cases:
	IMethod method = null;
// SH}
...
//{ObjectTeams: ignore enclosing team during searching in order to find tsuper/tsub versions, too.
				try {
			...
			if (isConstructor) {
				...
				searchPattern =
					new MethodPattern(
						...
						limitTo,
						matchRule);
//{ObjectTeams: only search for forward-references:
				if (element instanceof CalloutMapping)
					((MethodPattern)searchPattern).constrainToCallerDirection = true;
				// store declaring role class in order to enable checking tsub/tsuper lateron:
				if (declaringRoleClass != null)
