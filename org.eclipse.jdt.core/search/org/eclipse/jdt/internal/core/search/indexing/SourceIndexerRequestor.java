 ...
 * $Id: SourceIndexerRequestor.java 22561 2009-09-21 22:18:08Z stephan $
 * 
 * Contributors:
...
//{ObjectTeams:	for team package (do we want to store this info?)
    if ((importReference.modifiers & ClassFileConstants.AccTeam) != 0)
    {
        ...
        this.indexer.addTypeReference(typeName);
    }
// SH + gbr}
...
//{ObjectTeams: handle baseclass essentially like superclass:
	if (typeInfo.baseclassName != null)
		typeInfo.baseclassName = getSimpleName(typeInfo.baseclassName);
// SH}
...
/* orig:
  ...
  :giro */
	this.indexer.addClassDeclaration(typeInfo.modifiers, this.packageName, typeInfo.name, typeNames, typeInfo.superclass, typeInfo.superinterfaces, typeInfo.baseclassName, typeParameterSignatures, typeInfo.secondary);
// SH}
	addDefaultConstructorIfNecessary(typeInfo);
	pushTypeName(typeInfo.name);
}
...
//{ObjectTeams: enterInterface and enterClass merged here so:
	enterOTTypeInfo(typeInfo.name, typeInfo.isRoleFile);
//km:merge}
}
...
//{OTDTUI: implementation of OT extension to ISourceElementRequestor
private void enterOTTypeInfo(char[] name, boolean isRoleFile) {
	//TODO(ak): extend indexing support for teams/roles
	if (isRoleFile)
	{
	    this.indexer.addReferenceToTeamPackage(this.packageName, name);
	}
}

//TODO(carp): do we want to index callins and callouts? what about parameter mappings?
...
public void enterCallinMapping(CallinInfo callinInfo)
{
	...
	for (int i = 0; i < callinInfo.right.length; i++) {
		...
		this.indexer.addMethodReference(baseInfo.selector, argCount);
	}
}
...
{
    char[] baseClassName = CharOperation.concatWith(typeName, '.');
    this.indexer.addBaseReference(baseClassName);
}
//ak,gbr,haebor,SH}
}
