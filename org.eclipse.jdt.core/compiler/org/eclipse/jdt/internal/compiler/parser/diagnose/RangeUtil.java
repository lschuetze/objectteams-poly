 ...
 * $Id: RangeUtil.java 23404 2010-02-03 14:10:22Z stephan $
 *
 * Contributors:
...
//{ObjectTeams:
			//method mappings: (mainly copied from method treatment above)
			AbstractMethodMappingDeclaration[] mappings = types[j].callinCallouts;
			if (mappings != null) {
				int length = mappings.length;
				for (int i = 0; i < length; i++) {
					AbstractMethodMappingDeclaration mapping = mappings[i];
					if (mapping.mappings != null) {
						// TODO(SH) equivalent of containsErrorInSignature above?
						result.addInterval(mapping.bodyStart, mapping.bodyEnd, IGNORE);
					}
				}
			}
// SH}
