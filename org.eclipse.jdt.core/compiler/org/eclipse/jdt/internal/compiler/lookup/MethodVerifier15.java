...
//{ObjectTeams: callins need no bridge, return is Object in any case:
   ...
   // orig:
     ...
     //{third level change: conflicting callin<->non-callin are not treated as overriding:
					if (inheritedMethod != null) {
						checkCallinModifierConflict(currentMethod, inheritedMethod);
...
/**
* Check compatibility of callin method between current and inherited.
* @param currentMethod
* @param inheritedMethod
*/
void checkCallinModifierConflict(MethodBinding currentMethod, MethodBinding inheritedMethod) {
	if (   !currentMethod.declaringClass.isSynthInterface()
		&& !inheritedMethod.isPrivate())
	{
		if (currentMethod.isCallin())
		{
			// if this is just the class part overriding the ifc version, skip it
			RoleModel role = currentMethod.declaringClass.roleModel;
			// Note (SH): If !isRole(), a wrapper (residing in the team) is being redefined.
				...
				&& inheritedMethod.declaringClass != role.getInterfacePartBinding())
			{
				...
				AbstractMethodDeclaration methodDecl = currentMethod.sourceMethod();
				 // enhancement fake has no source (see CallinImplementor.createEnhancedBinding())
				if (methodDecl != null)
					MethodModel.addCallinFlag(methodDecl, IOTConstants.CALLIN_FLAG_OVERRIDING);
			}
			// callin overriding regular?
