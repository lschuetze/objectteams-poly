...
//{ObjectTeams: nested type of method mapping (via param map)?
	if (anonymousType.binding != null) {
		...
		if (   methodScope != null 
			...
			// help isRole() by setting isRolish manually:
...
//{ObjectTeams:
	if (isUninitializedBaseField(binding))
		return true;
// SH}
...
//{ObjectTeams: transitively follow assignment to field _OT$base
private boolean isUninitializedBaseField(FieldBinding field) {
	...
	if (methodScope().referenceContext instanceof ConstructorDeclaration) {
		ConstructorDeclaration ctor = (ConstructorDeclaration)methodScope().referenceContext;
		return !MethodModel.callsBaseCtor(ctor.constructorCall.binding);
	}
	return false;
}
// SH}
...
//{ObjectTeams: don't report unused generated locals:
...
/* orig:
   ...
   :giro */
			if (CharOperation.equals(sourceType.internalName(), name))
// SH}
				return sourceType;
		}
	}
...
//{ObjectTeams
/**
 * Answer the ClassScopes of all local types defined in this scope.
 */
public ClassScope[] getAllLocalTypes() {
...
private LinkedList<ClassScope> getAllLocalTypesInternal() {
    LinkedList<ClassScope> list = new LinkedList<ClassScope>();
    if (this.subscopes == null) return list;
    for (int i=0; i<this.subscopes.length; i++) {
        Scope sub = this.subscopes[i];
        if (sub instanceof ClassScope)
            list.add((ClassScope)sub);
        else if (sub instanceof BlockScope)
            list.addAll(((BlockScope)sub).getAllLocalTypesInternal());
    }
    return list;
}
// SH}
...
//{ObjectTeams: extract original method to add 2. strategy in case of NotFound:

	// using this order should maintain original performance for the usual case.

	Binding result = internalGetBinding(compoundName, mask, invocationSite, needResolve);
	if (result.isValidBinding() || result.problemId() != ProblemReasons.NotFound)
		return result;

	// from this point only NotFound situations:

	// handle the case of externalized roles "prefix.Type"
	// (similar to Scope.getType(char[][], int)).
	if (compoundName.length >= 2 && ((mask & Binding.TYPE) != 0)) {
		TypeBinding typeBinding = RoleTypeCreator.resolveAnchoredType(this, null, compoundName, 0);

		if (typeBinding != null) {
			if (   typeBinding.isValidBinding()
				|| (mask & ~Binding.TYPE) == 0) // otherwise original problem might be more appropriate
			{
...
private Binding internalGetBinding(char[][] compoundName, int mask, InvocationSite invocationSite, boolean needResolve)
{
// SH}
...
//{ObjectTeams: record referenced teams, no matter how the type was found:
		TypeModel.checkReferencedTeam(binding, this);
// SH}
		return binding;
	}
...
//{ObjectTeams: consider static role methods:
	// similar to "if (insideConstructor)" above.
...
//{ObjectTeams: added one more check (static role methods are handled above and below)
/* orig:
	if (currentMethodScope.isStatic) {
  :giro */
	if (currentMethodScope.isStatic && !sourceType.isRole()) {
...
//{ObjectTeams: consider (again) static role methods:
	  if (currentMethodScope.isStatic && sourceType.isRole())
	  {
		  SyntheticArgumentBinding syntheticArg;
...
//{ObjectTeams: two dimensions for travelling
		  // 1. role->base
		  if (currentType.isRole() && currentType.baseclass() != null)
			syntheticField = currentType.getField(IOTConstants._OT_BASE, true);
		  if (   syntheticField != null
			  && syntheticField.isValidBinding()
			  && TypeAnalyzer.sameOrContained(currentType.baseclass(), targetEnclosingType))
		  {
		    // if successful mark how far we have come:
			currentEnclosingType = currentType.baseclass();
		  } else
		  // 2. inner->outer
// SH}
...
//{ObjectTeams: added 4. arg: externalizedReceiver
			path[count++] = ((SourceTypeBinding) syntheticField.declaringClass).addSyntheticMethod(syntheticField, true/*read*/, false /*not super access*/, true);
// SH}
