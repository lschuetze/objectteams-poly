 ...
 *
 * OTDT changes:
 * What: write OT-specific class attributes + Synthetic
 *
 * What: change sourceName() -> internalName()
 * Why:  avoid beautified name of role classes
 *
 * What: isRequiredToClearPrivateModifier also clears AccProtected for interface methods.
 * Why:  role ifc methods can have any modifiers, but they must be public in the byte code.
 *
 * What: allow resizing of the contents from AbstractAttribute and its subclasses.
 *
...
//      See also TypeDeclaration.generateCode().
//		Note that a converted role file may be a membertype of another (non-converted) type.
...
//{ObjectTeams: clear private/protected for fields in synthetic role interfaces:
		if ((fieldBinding.tagBits & TagBits.ClearPrivateModifier) != 0) {
            accessFlags &= ~(ClassFileConstants.AccPrivate | ClassFileConstants.AccProtected);
...
//{ObjectTeams: write OT-specific byte code attributes
        if (fieldBinding.model != null)
            attributeNumber += fieldBinding.model.writeAttributes(this);
// SH}
...
//{ObjectTeams: fields for value parameters (don't bother to generate AST for those)
		for (SyntheticArgumentBinding argBinding : currentBinding.valueParamSynthArgs())
			if (argBinding.matchingField != null)
				addFieldInfo(argBinding.matchingField);
// SH}
	...
	public void addSyntheticMethodAccessMethod(SyntheticMethodBinding methodBinding) {
		...
		completeCodeAttributeForSyntheticMethod(
			...
			((SourceTypeBinding) methodBinding.declaringClass)
				...
				.compilationResult
				.getLineSeparatorPositions());
//{ObjectTeams: base call surrogate may have callin flags to store the original return type
		if (methodBinding.model != null)
			attributeNumber += methodBinding.model.writeAttributes(this);
// SH}
		// update the number of attributes
		this.contents[methodAttributeOffset++] = (byte) (attributeNumber >> 8);
		this.contents[methodAttributeOffset] = (byte) attributeNumber;
...
//{ObjectTeams: role methods may need to be rewritten to yield valid bytecode:
		  int rewrittenFlags = MethodModel.rewriteModifiersForBytecode(methodBinding);
		  if (rewrittenFlags != -1)
			accessFlags = rewrittenFlags;
		  else
// SH}
...
//{ObjectTeams: synthetic enclosing team + dummy arg?
	        if (methodBinding.needsSyntheticEnclosingTeamInstance()) {
	        	frame.putLocal(resolvedPosition, new VerificationTypeInfo(VerificationTypeInfo.ITEM_OBJECT, methodBinding.declaringClass.enclosingType()));
	        	resolvedPosition++;
...
//{ObjectTeams: byte code attributes need contents resizing:
	public final byte[] getResizedContents(int minimalSize) {
		resizeContents(minimalSize);
...
//{ObjectTeams: helpers for storing the path where a classfile is written to
	// This is the flow of information:
	// 1. CompilationUnitDeclaration.cleanUp() gives us a chance to store the type model
	//    (by calling maybeRememberModel())
	// 2. AbstractImageBuilder.writeClassFile() tells us about the file path
	//    (by calling maybeSetClassFilePath())
	//    The same for Util.getFileOutputStream().
	// 3. MethodModel.setupByteCode() may use this path to read the byte code from disc.
	 ...
	 */
	public void maybeRememberModel() {
		if (this.referenceBinding != null) {
			if (this.referenceBinding.isRole())
				this.generatingModel = this.referenceBinding.roleModel;
				// role model's setClassFilePath() will transfer the path to a team model, too, if present.
			else if (this.referenceBinding.isTeam())
				this.generatingModel = this.referenceBinding.getTeamModel();
		}
	}
	public void maybeSetClassFilePath(String path) {
		if (this.generatingModel != null) {
			this.generatingModel.setClassFilePath(path);
			this.generatingModel = null; // cleanup, reference is no longer needed.
		}
	}
	/**
     * Releasing the role model means, that this class file can no longer be used
     * to retrieve that role's bytes.
     */
	void releaseModel() {
		if (this.generatingModel instanceof RoleModel)
			((RoleModel)this.generatingModel).releaseClassFile();
		this.generatingModel= null;
	}
// SH}
}
