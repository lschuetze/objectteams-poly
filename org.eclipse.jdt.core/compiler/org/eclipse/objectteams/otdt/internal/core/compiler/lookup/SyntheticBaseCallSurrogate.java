 ...
 * Unter OTREDyn we generate a default base-call surrogate that simply delegates to _OT$callNext(..).
 * 
 * @author stephan
 * @since 1.3.0
 */
public class SyntheticBaseCallSurrogate extends SyntheticOTMethodBinding
{
	...
	private ReferenceBinding objectType;
	private MethodBinding callNext;
	...
	public SyntheticBaseCallSurrogate(MethodBinding callinMethod, SourceTypeBinding declaringClass) {
		super(declaringClass, ClassFileConstants.AccPublic|ClassFileConstants.AccSynthetic, callinMethod.selector, callinMethod.parameters, callinMethod.returnType);
		...
		this.objectType = scope.getJavaLangObject();
		this.callNext = scope.getOrgObjectteamsITeam().getMethods(CallinImplementorDyn.OT_CALL_NEXT)[0];
	...
	public void generateInstructions(CodeStream codeStream) {
		if (CallinImplementorDyn.DYNAMIC_WEAVING) {
			...
			if (this.targetMethod.isStatic()) {
				codeStream.aload_0(); // synthetic team arg
			} else {
				ReferenceBinding teamType = this.targetMethod.declaringClass.enclosingType();
				AbstractMethodDeclaration targetMethodDecl = this.targetMethod.sourceMethod();
				if (targetMethodDecl != null) {
					BlockScope scope = targetMethodDecl.scope;
					Object[] path = scope.getEmulationPath(teamType, true /*only exact match*/, false/*consider enclosing arg*/);
					codeStream.generateOuterAccess(path, null/*invocationSite*/, teamType, scope);
				}
			}
			TypeBinding[] targetParameters = this.targetMethod.parameters;
			int numSynthArgs = 6; // base, teams, idx, callinIds, boundMethodId, args
			// base instance & enhancement args:
			for(int i=0; i<numSynthArgs; i++)
				codeStream.load(targetParameters[i], i+1); // position 0 is this or enclosing team
			// packed base-call arguments
			codeStream.generateInlinedValue(targetParameters.length-numSynthArgs);
			codeStream.anewarray(this.objectType);
			for(int i=numSynthArgs; i<targetParameters.length; i++) {
				codeStream.dup();
				codeStream.generateInlinedValue(i-numSynthArgs);
				codeStream.load(targetParameters[i], i+2); // position 0 is this or enclosing team, also skip baseSuper flag (FIXME)
				if (targetParameters[i].isBaseType())
					codeStream.generateBoxingConversion(targetParameters[i].id);
				codeStream.aastore();
			}
			codeStream.invoke(Opcodes.OPC_invokeinterface, this.callNext, this.callNext.declaringClass);
			// decoding return:
			TypeBinding realReturnType = MethodModel.getReturnType(this.targetMethod);
			if (realReturnType.id != TypeIds.T_void) {
				if (realReturnType != this.objectType) {
					if (realReturnType.isBaseType()) {
						char[][] boxtypeName= AstGenerator.boxTypeName((BaseTypeBinding)realReturnType);
						Scope scope = this.targetMethod.sourceMethod().scope;
						codeStream.checkcast(scope.getType(boxtypeName, 3));
					} else {
						codeStream.checkcast(realReturnType);
					}
				}
			}
			codeStream.areturn();
